...
def search_journey(request):
    ...
    try:
        ...
        if result is None:
            ...
            if bool(journey_dep_station) | bool(journey_ret_station):
                ...
                if journey_dep_station:
                    q &= Q('match_phrase_prefix', departure_station_name=journey_dep_station)
                if journey_ret_station:
                    q &= Q('match_phrase_prefix', return_station_name=journey_ret_station)

                if dates:
                    q &= Q('range', departure_time={'gte': dates[0] if dates[0] else datetime(2020, 1, 1, 00, 00, 00, 0).date(),
                    'lte': dates[1] if dates[1] else datetime.now().date()})

                if distance:
                    q &= Q('range', covered_distance={'gte': distance[0] if distance[0] else 10, 
                    'lte': distance[1] if distance[1] else max_distance})

                if duration:
                    q &= Q('range', duration={'gte': duration[0] if duration[0] else 10,
                    'lte': duration[1] if duration[1] else max_duration})
        ...
        context.update({
            ...
            'search_results_count' : len(result.hits), 
            'page_obj' : page_obj,
...
#         multiple_q = Q()
#         search = Search(index='journeys')
#         search = search.query("match", departure_station_name=journey_dep_station) | search.query("match", return_station_name=journey_ret_station)
